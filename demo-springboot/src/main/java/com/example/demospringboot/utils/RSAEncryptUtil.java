...
public class RSAEncryptUtil {
    public static void main(String[] args) throws Exception {
        ...
        String messageEn = encrypt(message, integerStringMap.get(0));
        System.out.println(message + "\t加密后的字符串为:" + messageEn);
        String messageDe = decrypt(messageEn, integerStringMap.get(1));
     ...
     * 加签过程
     */
    public static String signature(String privateKey, String data) throws Exception {
        //base64编码的私钥
     ...
     * 验签过程
     */
    public static boolean verify(String publicKey, String signed , String data) throws Exception {
        byte[] decoded = Base64.decodeBase64(publicKey);
        PublicKey pubKey = KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(decoded));
        Signature signature = Signature.getInstance("Sha1WithRSA");
        signature.initVerify(pubKey);
        signature.update(data.getBytes());
        boolean verify = signature.verify(Base64.decodeBase64(signed));
        return verify;
    }
